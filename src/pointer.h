...
protected:
	...
	void ptrDetach(void)
	{
		if(ptrCount && --(*ptrCount)==0) {
			/*cout.unsetf( ios::dec );
			cout.setf( ios::hex );
			cout<<"deleting - 0x"<<(int)ptrObject<<endl;
			cout.setf( ios::dec );*/
			if(ptrObject != NULL){
				free(ptrObject);
			}
...
public:
	/**
	  *	Construct a new smart pointer which points to ptr. The provided
	  *	memory location MUST be allocated using malloc, otherwise memory
	  *	leaks WILL occur.
	  *
	  *	@param	ptr	Buffer to automatically delete
	  */
